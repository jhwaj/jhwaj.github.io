...
const SCENARIOS = [
  ...
  /* ── 특수: 빈 캔버스 ── */
  {
    ...
    scenes: [
      ...
      { bg: 'night', hold: 3400, fx: [], sub: '그래도 끝까지 본 당신. 진정한 본방 사수 시청자다. — 다음 주에 계속.' },
    ],
  },
  ...
  /* ── 2. 아홉 번째 양 — 양 3마리 이상 ── */
  {
    ...
    scenes: [
      ...
      { bg: 'night', hold: 3800,
        fx: [{ piece: 'sheep', anim: 'bob2' }],
        sub: '아홉 번째 양의 정체는 미궁에 빠졌다. 다만 그날 이후, 늑대 실종 신고가 한 건 접수됐다.' },
    ],
  },
  ...
  /* ── 7. 같은 정류장 — 우산 2개 이상 ── */
  {
    ...
    scenes: [
      ...
      { bg: 'dusk', hold: 3600,
        ...
        sub: '버스가 왔지만 둘은 타지 않았다. 3년 만에 다시, 나란히 접혔다. 회장님은 다음 화에서 응징된다.' },
    ],
  },
  ...
  /* ── 특수: 정의되지 않은 조합 — 조각들을 관계 비트 사슬로 엮은 한 편의 이야기 ── */
  {
    ...
    poster: { sky: '#3D7BE8', grass: '#4FA53B' },
    match: () => true,
    // 3막 구조: 시작(전원 소개 + 주인공의 사연) → 전개(합류 비트, 전원이 서서히 모임) → 결말(수렴 + 진실 + 다음 주 예고)
    buildScenes: c => {
      ...
      scenes.push({
        ...
        sub: '모두 부둥켜안고 울었다. 카메라 천천히 줌아웃. — 다음 주에 계속.',
      });
      return scenes;
    },
  },
...
const RESOLUTION_VARIANTS = {
  ...
  cloud: '가문은 결혼을 포기했다. 구름은 이 들판에 신접살림을 차렸다. 상대는 다음 화에 공개된다.',
